...
function parseHttpRequest(p, data, res) {
	...
	if (data.type === 'cookies') {
		...
		for (i = 0; i < p.config.commandList.length; i++) {
			...
			ret.push({source:{x:cmd.source.x, y:cmd.source.y, id:cmd.source.id, name:cmd.source.name}, target: cmd.target, troops: cmd.troops, time: cmd.time, id:cmd.id, sendTime:cmd.sendTime, support:cmd.support});
		}
		...
		for (i = 0; i < p.config.commandList.length; i++) {
			cmd = p.config.commandList[i];
			if (cmd.id === data.id) {
				return JSON.stringify({source:{x:cmd.source.x, y:cmd.source.y, id:cmd.source.id, name:cmd.source.name}, target: cmd.target, troops: cmd.troops, time: cmd.time, id:cmd.id, sendTime:cmd.sendTime, support:cmd.support});
			}
		}
